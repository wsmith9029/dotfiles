...
-- Panic DPMS
hl.bind(mainMod .. " + SHIFT + M", hl.dsp.dpms({ action = "enable" }))

-- Clipboard Menu
hl.bind(mainMod .. " + C", hl.dsp.exec_cmd("cliphist list | wofi --dmenu -p `Search:` | cliphist decode | wl-copy"))
...
-- Restart Waybar
hl.bind(mainMod .. " + SHIFT + B", hl.dsp.exec_cmd("pkill -SIGUSR2 waybar || waybar"))
...
hl.bind("XF86MonBrightnessUp",  hl.dsp.exec_cmd("./.config/waybar/scripts/brightness --inc"),                  { locked = true, repeating = true })
hl.bind("XF86MonBrightnessDown",hl.dsp.exec_cmd("./.config/waybar/scripts/brightness --dec"),                  { locked = true, repeating = true })
...
-- Apply opacity and blur to Kitty terminal windows natively
hl.window_rule({
    match = { class = "^(kitty)$" },
    no_blur = false,
    opacity = "0.9 0.75",
